...
function isYoutubeUrl(url) { return !!extractYoutubeId(url); }

// ── Custom TipTap node: VideoNode (handles both uploaded mp4 and YouTube) ─────
const VideoNode = Node.create({
  ...
  addAttributes() {
    return {
      src: { default: null },
      type: { default: "upload" }, // "upload" | "youtube"
  ...
  // In-editor NodeView: YouTube shows thumbnail + play icon (no live iframe = no bot check)
  // Uploaded video shows a real <video> tag
  addNodeView() {
    return ({ node }) => {
      const { src, type, youtubeId } = node.attrs;
...
// ── Custom TipTap node for side-by-side images ────────────────────────────────
...
// ── Shared helper to insert a VideoNode into the editor ───────────────────────
function insertVideoNode(editor, { src, type, youtubeId }) {
  editor.chain().focus().insertContent({
    type: "videoNode",
    attrs: { src, type, youtubeId },
  }).run();
}
...
// ── Video modal: URL tab + Upload tab ────────────────────────────────────────
function VideoModal({ editor, onClose }) {
  ...
  const handleUrl = () => {
    ...
    if (ytId) {
      ...
      // Treat as direct video URL (e.g. Vimeo direct link, CDN link, etc.)
...
function SideBySideModal({ editor, onClose }) {
  ...
  const insert = async () => {
    ...
    try {
      ...
      editor.chain().focus().insertContent({
        type: "imageRow",
        attrs: { urls, gap: parseInt(gap, 10) },
      }).run();
      onClose();
    } catch (err) {
      alert(`Upload failed: ${err.message}`);
...
export default function CreatePost() {
  ...
  if (isLoadingAuth) return <div className="fixed inset-0 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div>;
